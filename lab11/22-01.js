...
const token = "6366838619:AAFecWlHJAwmABOycCtSYHz126en_iY5i7Q";
const request = require("request");
const bot = new TelegramBot(token, { polling: true });
const THE_CAT_API_KEY =
  "live_UjP8C4WO6HYxlI1TKFpmfU5L0rGrOT7qpWMpIxnvjHnHAOBADF6zhIb2cUp41Vzo";
...
bot.onText("", async (msg) => {
  ...
  if (msg.text.toLowerCase() === "привет") {
    const stickerFileId =
      "CAACAgIAAxkBAAEE_EVmKoXNodrM5U5oA0f4IZLFAfzB8gACfggAAlwCZQPp3EGKQYqWpDQE";
  ...
  if (msg.text.toLowerCase() === "/subscribe") {
    ...
    prismaClient.subscribers
      ...
      .catch((error) => {
        if (error.code === "P2002") {
          bot.sendMessage(chatId, "Ошибка при подписке");
        } else {
  ...
  if (msg.text.toLowerCase() === "/unsubscribe") {
    prismaClient.subscribers
      ...
      .catch((error) => {
        ...
        bot.sendMessage(chatId, "Ошибка при отписке");
      });
  }
  ...
  if (msg.text.toLowerCase() === "/joke") {
    request.get(
      ...
      function (error, response, body) {
        if (error) {
          bot.sendMessage(chatId, "Ошибка при запросе шутки");
          return console.error("Request failed:", error);
        } else if (response.statusCode != 200) {
          bot.sendMessage(
            chatId,
            "Не удалось получить шутку: " + response.statusCode
          );
          ...
          try {
            ...
            bot.sendMessage(chatId, "Ошибка при обработке шутки");
  ...
  if (msg.text.toLowerCase() === "/cat") {
    axios
      ...
      .catch((error) => {
        bot.sendMessage(chatId, "Ошибка при получении изображения кота");
        console.error(error);
      });
  }
});
...
function sendFact(chatId) {
  request.get(
    ...
    function (error, response, body) {
      if (error) {
        bot.sendMessage(chatId, "Ошибка при запросе фактов");
        return console.error("Request failed:", error);
      } else if (response.statusCode != 200) {
        bot.sendMessage(
          chatId,
          "Не удалось получить факты: " + response.statusCode
        );
        return console.error(
          "Error:",
          response.statusCode,
        ...
        try {
          ...
          bot.sendMessage(chatId, "Ошибка при обработке фактов");
          return console.error("Parse failed:", parseError);
        }
      }
    }
